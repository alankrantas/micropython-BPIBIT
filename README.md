...
Beware that this module also eats more memory than usual MicroPython libraries.
...
```python
value = BPIBIT.acceleration('x')
value = BPIBIT.rotationPitch()
value = BPIBIT.rotationRoll()
value = BPIBIT.gyroscope('y')
value = BPIBIT.magneticForce('z')
value = BPIBIT.compassHeading()
...
### Scroll Pictures ON LED Display

This function allows you to scroll multiple pictures (which defined in arrays with color codes) across the LED screen, speed adjustable (default 150 ms).
...
BPIBIT.scrollPics(arrays, delay=150)
```
